...
# Import miniDeepConus
from dataset.mini_deep_conus import DeepConus, MetaDeepConus
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='miniImageNet', choices=['miniImageNet', 'tieredImageNet',
                                                                                'CIFAR-FS', 'FC100', 'miniDeepConus']) # Added miniDeepConus
    ...
    elif opt.dataset != 'miniDeepConus': # Once again I do not want this
    ...
    # Windows filenames can't use colons, so : --> -
...
def main():
    ...
    if opt.dataset == 'miniImageNet':
        ...
        # Same thing, copy-paste, change to DeepConus
        train_trans, test_trans = transforms_options[opt.transform]
        if opt.distill in ['contrast']:
            train_set = DeepConus(args=opt, partition=train_partition, transform=train_trans, is_sample=True, k=opt.nce_k)
        else:
            train_set = DeepConus(args=opt, partition=train_partition, transform=train_trans)
        n_data = len(train_set)
        train_loader = DataLoader(train_set,
                                  batch_size=opt.batch_size, shuffle=True, drop_last=True,
                                  num_workers=opt.num_workers)
        ...
        meta_testloader = DataLoader(MetaDeepConus(args=opt, partition='test',
                                     ...
                                     batch_size=opt.test_batch_size, shuffle=False, drop_last=False,
                                     num_workers=opt.num_workers)
        meta_valloader = DataLoader(MetaDeepConus(args=opt, partition='val',
                                                 train_transform=train_trans,
                                                 test_transform=test_trans),
                                    batch_size=opt.test_batch_size, shuffle=False, drop_last=False,
                                    num_workers=opt.num_workers)
    ...
    data = torch.randn(2, 23, 32, 32) # This was (2, 3, 84, 84)
    model_t.eval()
    model_s.eval()
...
def train(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in tqdm(enumerate(train_loader), desc="Distillation epoch " + str(epoch)):
        if opt.distill in ['contrast']:
    ...
    # Removed printouts in loop, added avg loss
